...
def expense_summary(month=None):
    ...
    expenses = load_expenses()
    if month:
        expense = [exp for exp in expenses if datetime.strptime(exp["date"], "%Y-%m-%d").month == month]
...
def main():
    ...
    parser.add_argument("action", type=str, choices=["add", "update", "delete", "list", "summary"], help="Action to perform")
    ...
    match args.action:
        ...
        case "update":
            if args.id and args.description and args.amount:
                ...
                print("ID, description, and amount are required for updating an expense.")
        case "delete":
            if args.id:
